...
class STTHandler:
    ...
    def listen(self):
        ...
        self._listening_thread = threading.Thread(target=self._listen_loop)
        self._listening_thread.start()

    def _listen_loop(self):
        while not self._stop_event.is_set():
            # 음성 데이터 수집 및 처리 로직
            # 예: 마이크에서 음성 받아서 인식하는 코드
            pass
        self.is_listening = False  # 루프 종료 시 상태 변경

    def stop(self):
        self._stop_event.set()  # 루프 종료 신호
...
class STT:
    ...
    def stop(self):
        ...
        if self.listen_thread and self.listen_thread.is_alive():
            self.listen_thread.join()  # 녹음 쓰레드가 종료될 때까지 기다림
            print("녹음 쓰레드 정상 종료")
...
def stop_stt():
    stt.stop()  # 위에서 stop()이 제대로 루프 종료 및 상태 변경을 담당
