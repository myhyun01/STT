    ...
    def transcribe(self):
        while self.is_listening:
            ...
            if audio_data == 'STOP':
                break
            # 항상 한국어로 인식
            segments, info = self.model.transcribe(audio_data, beam_size=5, vad_filter=True, language="ko")
    ...
    def listen(self):
        def background():
            ...
            with mic as source:
                ...
                print("짧은 청크로 음성 인식 시작!")
                while self.is_listening:
                    try:
                        audio_data = self.recorder.listen(source, phrase_time_limit=0.5)
                        audio = io.BytesIO(audio_data.get_wav_data())
                        self.data_queue.put(audio)
                        time.sleep(0.1)
                    except Exception as e:
                        logging.warning(f"녹음 중 에러: {e}")
                        continue
